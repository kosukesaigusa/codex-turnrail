#!/usr/bin/env python3
"""Validate product versions and generate the release's Codex reference metadata."""
...
VERSION_GENERATED = Path("app/Sources/CodexTurnrailApp/TurnrailVersion.generated.swift")
...
def product_version_swift(version):
    return (
        "// Generated by scripts/project_metadata.py from packaging/Info.plist.\n"
        "\n"
        "enum TurnrailVersion {\n"
        f"  static let current = {json.dumps(version)}\n"
        "}\n"
    )


def write_product_version(root):
    version, _ = product_version(root)
    (root / VERSION_GENERATED).write_text(product_version_swift(version))
...
def validate(root, *, tag=None):
    ...
    if (root / VERSION_GENERATED).read_text() != product_version_swift(version):
        raise ValueError("Turnrail app version is stale; run just metadata-write.")
