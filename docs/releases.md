...
The build manifest records the product and upstream versions, tagged app source commit, Swift compiler, signer, app/router hashes, and notarization report hash. Its official Engine record contains the reference app identity, OpenAI signing team, and hashes of the separately installed official CLI and Host used during verification. Archive creation rejects uncommitted source, another source revision, changed binaries or resources, failed or incomplete reports, unnotarized apps, and version mismatches. It verifies the attached ticket and Gatekeeper assessment again before creating the ZIP.
