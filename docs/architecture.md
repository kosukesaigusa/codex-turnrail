...
Every upstream connection also sends a keepalive ping every twenty seconds, including between requests and while awaiting model output. Foreground and periodic probes are serialized. Missing pongs terminate the connection and wake pending operations; they do not submit or replay inference. Model output retains its separate 180-second receive deadline, which pongs cannot extend. Closure, timeouts, and Foundation callbacks complete each waiter once, preserve the first failure, and prevent a closed connection from passing another probe.
