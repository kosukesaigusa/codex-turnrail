...
The user subsequently reported no recurrence and authorized preparation of the Draft Release. This observation does not establish a specific tested idle duration or long-term reliability. Remaining desktop checks include automatic desktop title creation, real-account switching, and Settings quit behavior on the downloaded release package. The classified safety stop still requires review of the affected task; correcting its protocol classification is not evidence that the original task can resume. Local Swift builds previously used the user's explicit exception to the 30 GiB free-space requirement; the repository's default requirement remains unchanged.
