...
- `just package /absolute/output/directory 'Signing Identity' /absolute/official/codex release` builds, signs, and verifies the complete distribution app, then cleans generated build artifacts. Use `dev-small` explicitly for a development package. Both source paths are derived from this repository.
- Distribution builds use the existing upstream `release` profile through `scripts/dev.py release-build`, with the same lock, free-space reserve, and incremental-compilation policy. Ordinary Engine development commands retain `dev-small`.
