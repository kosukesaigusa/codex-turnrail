...
impl AgentControl {
    ...
    pub(crate) async fn ensure_v2_agent_loaded(
        ...
        let auth_manager = match parent.as_ref() {
            Some((parent, _)) => Arc::clone(&parent.session.services.auth_manager),
            None => state.auth_manager_for_parent(parent_thread_id).await?,
        };
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let auth_manager = state.auth_manager_for_parent(parent_thread_id).await?;
