...
impl ThreadManager {
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> CodexResult<NewThread> {
        self.start_thread_with_auth_manager(options, Arc::clone(&self.state.auth_manager))
            .await
    }

    /// Starts a root thread whose runtime is bound to the provided authentication manager.
    ///
    /// The authentication manager is fixed for the lifetime of the loaded runtime. Callers that
    /// need to change accounts must stop the runtime and resume its persisted history with the
    ...
    /// Replaces a stopped ephemeral runtime without depending on persisted history.
    pub async fn resume_ephemeral_thread_with_auth_manager(
        &self,
        source: &CodexThread,
        auth_manager: Arc<AuthManager>,
        parent_trace: Option<W3cTraceContext>,
        ...
        if source.is_running() {
            return Err(CodexErr::InvalidRequest(
                "ephemeral runtime must be stopped before replacing its authentication".to_string(),
            ));
        }
        let initial_history = source.session.ephemeral_resume_history().await?;
        let config = source.config().await.as_ref().clone();
    ...
    where
        S: Into<ForkSnapshot>,
    {
        self.fork_thread_from_history_with_auth_manager(
            snapshot,
            config,
            history,
            thread_source,
    ...
    /// Forks already-loaded history into a runtime bound to the provided authentication manager.
    #[allow(clippy::too_many_arguments)]
    ...
    pub async fn fork_prepared_thread(
        ...
        self.fork_prepared_thread_with_auth_manager(
            config,
            prepared,
            thread_source,
            parent_trace,
    ...
    /// Forks prepared history into a runtime bound to the provided authentication manager.
    #[allow(clippy::too_many_arguments)]
    pub async fn fork_prepared_thread_with_auth_manager(
        &self,
        config: Config,
        prepared: PreparedFork,
        thread_source: Option<ThreadSource>,
...
impl ThreadManagerState {
    ...
    pub(crate) async fn spawn_new_thread_with_source(
        ...
        let auth_manager = self.auth_manager_for_parent(parent_thread_id).await?;
        let mut request = ThreadSpawnRequest::new(options, auth_manager, agent_control);
    ...
    pub(crate) async fn auth_manager_for_parent(
        &self,
        parent_thread_id: Option<ThreadId>,
    ) -> CodexResult<Arc<AuthManager>> {
        let Some(parent_thread_id) = parent_thread_id else {
            return Ok(Arc::clone(&self.auth_manager));
        };
        let parent = self.get_thread(parent_thread_id).await?;
        Ok(Arc::clone(&parent.session.services.auth_manager))
    }
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> CodexResult<NewThread> {
        ...
        let mut replaced_ephemeral_thread = None;
        if let InitialHistory::Resumed(resumed) = &initial_history {
            ...
            if let Some(thread) = threads.get(&resumed.conversation_id).cloned() {
                ...
                if config.ephemeral {
                    replaced_ephemeral_thread = Some(thread);
                } else {
        ...
        if let Some(source) = &replaced_ephemeral_thread {
            session
                .restore_ephemeral_context_from(&source.session)
                .await;
        }
        let new_thread = self
    ...
    async fn finalize_thread_spawn(
        ...
        io: SessionIo,
        session_source: SessionSource,
        replaced_ephemeral_thread: Option<Arc<CodexThread>>,
    ) -> CodexResult<NewThread> {
        ...
        {
            let mut threads = self.threads.write().await;
            let entry = threads.entry(thread_id);
            let can_insert = match &entry {
                std::collections::hash_map::Entry::Vacant(_) => replaced_ephemeral_thread.is_none(),
                std::collections::hash_map::Entry::Occupied(entry) => replaced_ephemeral_thread
                    .as_ref()
                    .is_some_and(|source| Arc::ptr_eq(source, entry.get())),
            };
            if can_insert {
