...
#[cfg(unix)]
#[test]
fn execve_review_payload_excludes_internal_completion_tracking() -> anyhow::Result<()> {
    let status = ExecApprovalStatus::new();
    let action = ApprovalAction::Execve {
        approval_status: status.clone(),
        id: "parent-command".to_string(),
        approval_id: "child-command".to_string(),
        environment_id: "local".to_string(),
        source: GuardianCommandSource::UnifiedExec,
        program: AbsolutePathBuf::try_from("/usr/bin/true")?,
        argv: vec!["true".to_string()],
        command: vec!["true".to_string()],
        cwd: AbsolutePathBuf::try_from("/tmp")?,
        additional_permissions: None,
    };
    let expected = serde_json::json!({
        "tool": "execve",
        "id": "parent-command",
        "approval_id": "child-command",
    ...
    status.record(&ReviewDecision::Abort);
