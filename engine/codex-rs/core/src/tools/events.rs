...
pub(crate) enum ToolEventFailure<'a> {
    Output(ExecToolCallOutput),
    RejectedOutput(ExecToolCallOutput),
    Message(String),
    Rejected {
        message: String,
        applied_patch_delta: Option<&'a AppliedPatchDelta>,
...
impl ToolEmitter {
    ...
    pub async fn emit(&self, ctx: ToolEventCtx<'_>, stage: ToolEventStage<'_>) {
        match (self, stage) {
            ...
            (
                Self::ApplyPatch { changes, .. },
                ToolEventStage::Failure(ToolEventFailure::RejectedOutput(output)),
            ) => {
                emit_patch_end(
                    ctx,
                    changes.clone(),
...
async fn emit_exec_stage(
    ...
    let declined = matches!(
        stage,
        ToolEventStage::Failure(ToolEventFailure::RejectedOutput(_))
    );
    match stage {
        ToolEventStage::Begin => {
