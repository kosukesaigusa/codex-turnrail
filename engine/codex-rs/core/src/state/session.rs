...
/// Owned context that can cross a runtime boundary without retaining a state lock.
...
impl SessionState {
    pub(crate) fn ephemeral_context_snapshot(&self) -> EphemeralContextSnapshot {
        EphemeralContextSnapshot {
            history: self.history.clone(),
            base_instructions_provenance: self.base_instructions_provenance.clone(),
            ...
            current_time_reminder: self.current_time_reminder.clone(),
            active_connector_selection: self.active_connector_selection.clone(),
            granted_permissions_by_environment_id: self
                .granted_permissions_by_environment_id
                .clone(),
            next_turn_is_first: self.next_turn_is_first,
        }
    }

    /// Keeps conversation and permission state while replacing account-scoped services.
    pub(crate) fn restore_ephemeral_context(&mut self, snapshot: EphemeralContextSnapshot) {
        self.history = snapshot.history;
        self.base_instructions_provenance = snapshot.base_instructions_provenance;
        self.latest_token_usage_record = snapshot.latest_token_usage_record;
        self.server_reasoning_included = snapshot.server_reasoning_included;
        self.mcp_dependency_prompted = snapshot.mcp_dependency_prompted;
        self.additional_context = snapshot.additional_context;
        self.previous_turn_settings = snapshot.previous_turn_settings;
        self.auto_compact_window = snapshot.auto_compact_window;
        self.current_time_reminder = snapshot.current_time_reminder;
        self.active_connector_selection = snapshot.active_connector_selection;
        self.granted_permissions_by_environment_id = snapshot.granted_permissions_by_environment_id;
        self.next_turn_is_first = snapshot.next_turn_is_first;
    }
