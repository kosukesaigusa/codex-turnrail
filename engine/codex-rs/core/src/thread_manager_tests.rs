...
#[tokio::test]
async fn subagent_inherits_the_parent_runtime_auth_manager() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.codex_home = temp_dir.path().join("codex-home").abs();
    config.cwd = config.codex_home.abs();
    ...
    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        CodexAuth::from_api_key("manager-default"),
        config.model_provider.clone(),
        config.codex_home.to_path_buf(),
        Arc::new(codex_exec_server::EnvironmentManager::default_for_tests()),
    );
    let parent_auth = AuthManager::from_auth_for_testing(CodexAuth::from_api_key("parent-account"));
    let parent = manager
        .start_thread_with_auth_manager(
            StartThreadOptions::new(config.clone()),
            Arc::clone(&parent_auth),
        )
        .await
        .expect("start account-bound parent");
    let child = parent
        .thread
        .session
        .services
        ...
        .await
        .expect("spawn account-bound child");
    ...
    let child_thread = manager
        ...
        .await
        .expect("load account-bound child");
    assert!(Arc::ptr_eq(
        &parent_auth,
...
#[tokio::test]
async fn ephemeral_fork_uses_the_explicit_runtime_auth_manager() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.codex_home = temp_dir.path().join("codex-home").abs();
    config.cwd = config.codex_home.abs();
    ...
    assert!(Arc::ptr_eq(
        &fork_auth,
        &fork.thread.session.services.auth_manager
    ));
...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    let bound_auth_manager =
        AuthManager::from_auth_for_testing(CodexAuth::from_api_key("bound-internal-parent"));
    let parent = manager
        .start_thread_with_auth_manager(
            StartThreadOptions {
