...
async fn unified_exec_zsh_fork_parent_and_subcommand_approvals_preserve_denied_reads() -> Result<()>
{
    ...
    let command = format!("/bin/cat {denied_path:?}");
    ...
    let denied_path_arg = denied_path.to_string_lossy().into_owned();
    let approval = expect_exec_approval(&test, &denied_path_arg).await;
    assert_eq!(
        approval.command,
        vec!["/bin/cat".to_string(), denied_path_arg]
    );
    ...
    assert_ne!(
        result
            .exit_code
            .expect("denied read should produce an exit code"),
        0,
    ...
    match event {
        EventMsg::ExecApprovalRequest(approval) => {
            ...
            assert_eq!(
                ...
                "approval request should target the expected command argument"
            );
            approval
        }
