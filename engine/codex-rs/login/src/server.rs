...
use crate::expected_login_email::ExpectedLoginEmail;
...
pub struct ServerOptions {
    ...
    /// Browser login must verify this registered identity before replacing credentials.
    pub expected_email: Option<ExpectedLoginEmail>,
...
async fn process_request(
    ...
    match path.as_str() {
        "/auth/callback" => {
            ...
            {
                Ok(tokens) => {
                    if let Some(expected_email) = &opts.expected_email
                        && let Err(message) = expected_email.verify_id_token(&tokens.id_token)
                    {
                        return login_error_response(
                            message,
                            io::ErrorKind::PermissionDenied,
                            Some("account_identity_mismatch"),
                            /*error_description*/ None,
                        );
                    }
