...
const EXPECTED_LOGIN_EMAIL_ENV_VAR: &str = "CODEX_TURNRAIL_EXPECTED_EMAIL";
...
pub async fn login_with_chatgpt(
    ...
    auth_keyring_backend_kind: AuthKeyringBackendKind,
    auth_route_config: AuthRouteConfig,
    expected_email: Option<ExpectedLoginEmail>,
) -> std::io::Result<()> {
    ...
    opts.expected_email = expected_email;
...
async fn start_chatgpt_login(opts: ServerOptions) -> std::io::Result<LoginServer> {
    if opts.expected_email.is_none() {
...
pub async fn run_login_with_chatgpt(cli_config_overrides: CliConfigOverrides) -> ! {
    let expected_email =
        match parse_expected_login_email(std::env::var(EXPECTED_LOGIN_EMAIL_ENV_VAR)) {
            Ok(expected_email) => expected_email,
            Err(error) => {
                eprintln!("Invalid {EXPECTED_LOGIN_EMAIL_ENV_VAR}: {error}");
                std::process::exit(1);
            }
        };
...
fn parse_expected_login_email(
    value: Result<String, std::env::VarError>,
) -> std::io::Result<Option<ExpectedLoginEmail>> {
    match value {
        Ok(email) => ExpectedLoginEmail::parse(&email).map(Some),
        Err(std::env::VarError::NotPresent) => Ok(None),
        Err(std::env::VarError::NotUnicode(_)) => Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Expected login email must be valid Unicode.",
        )),
    }
}
