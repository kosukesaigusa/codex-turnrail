...
/// Fetches an authoritative catalog with the supplied account's authentication.
///
/// Transport, authentication, and decoding errors are returned to the caller;
/// no bundled or cached catalog is substituted for a failed request.
pub async fn fetch_remote_models(
    provider_info: ModelProviderInfo,
    auth_manager: Arc<AuthManager>,
    http_client_factory: HttpClientFactory,
) -> CoreResult<Vec<ModelInfo>> {
    let endpoint = OpenAiModelsEndpoint::new(provider_info, Some(auth_manager));
    let response = endpoint
        .list_models(env!("CARGO_PKG_VERSION"), http_client_factory)
        .await?;
    Ok(response.models)
}
