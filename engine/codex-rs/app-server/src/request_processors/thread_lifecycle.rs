            ...
            tokio::select! {
                ...
                event = conversation.next_event() => {
                    ...
                    if let EventMsg::TurnStarted(started) = &event.msg
                        && let Some(coordinator) = &turnrail_coordinator
                        && let Err(error) = coordinator
                            .record_turn_started(conversation_id, started.started_at)
                            .await
                    {
                        let message = format!("Could not save account Last used: {error:#}");
