...
pub(crate) struct CatalogRequestProcessor {
    ...
    pub(super) turnrail_coordinator: Option<Arc<crate::turnrail::TurnrailCoordinator>>,
}
...
impl CatalogRequestProcessor {
    pub(crate) fn new(
        ...
        turnrail_coordinator: Option<Arc<crate::turnrail::TurnrailCoordinator>>,
    ) -> Self {
        Self {
            outgoing,
            skills_watcher,
    ...
    pub(crate) async fn model_list(
        ...
        let models = match &self.turnrail_coordinator {
            Some(coordinator) => crate::account_models::common_models(coordinator, &self.config)
                .await
                .map_err(|error| {
                    internal_error(format!("Could not load Turnrail models: {error:#}"))
                })?,
            None => {
                supported_models(
        ...
        Self::list_models(models, params).map(|response| Some(response.into()))
    }
    ...
    fn list_models(
        ...
        let models: Vec<_> = models
            .into_iter()
            .filter(|model| include_hidden == Some(true) || !model.hidden)
            .collect();
