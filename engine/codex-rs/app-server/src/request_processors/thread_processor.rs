...
pub(crate) struct ThreadRequestProcessor {
    ...
    pub(super) turnrail_coordinator: Option<Arc<TurnrailCoordinator>>,
}
...
impl ThreadRequestProcessor {
    ...
    pub(crate) fn new(
        ...
        turnrail_coordinator: Option<Arc<TurnrailCoordinator>>,
    ) -> Self {
        Self {
            auth_manager,
            thread_manager,
    ...
    pub(crate) async fn prepare_turn_account(
        &self,
        request_id: &ConnectionRequestId,
        ...
        let Some(coordinator) = self.turnrail_coordinator.as_ref() else {
            return Ok(());
        };
        let thread_id = ThreadId::from_string(&params.thread_id)
            .map_err(|error| invalid_request(format!("invalid thread id: {error}")))?;
        ...
        let config = thread.config().await.as_ref().clone();
        let snapshot = thread.config_snapshot().await;
        let environments = resolve_turn_environment_selections(
            self.thread_manager.as_ref(),
            params.environments.clone(),
        )?;
        let cwd = match resolve_request_cwd(params.cwd.clone())? {
            Some(cwd) => cwd,
            None => match environments.as_ref().and_then(|selections| {
                selections
                    .iter()
                    .find(|selection| selection.environment_id == LOCAL_ENVIRONMENT_ID)
            }) {
                Some(local) => local.cwd.to_abs_path().map_err(|error| {
                    invalid_request(format!("invalid local routing directory: {error}"))
                })?,
                None => snapshot.cwd().clone(),
            },
        };
        let authentication = coordinator
            .selected_authentication(&config, &cwd)
            .await
            .map_err(|error| {
                internal_error(format!(
                    "failed to select the Codex Turnrail account: {error}"
                ))
            })?;
        if thread.has_active_turn().await {
            return Ok(());
        }
        let requested_model = match &params.collaboration_mode {
            Some(mode) => mode.model(),
            None => params.model.as_deref().unwrap_or(&snapshot.model),
        };
        crate::account_models::validate_turn_model(&authentication, &config, requested_model)
            .await
            .map_err(|error| invalid_request(format!("Cannot start this turn: {error:#}")))?;
        if thread.has_active_turn().await {
            return Ok(());
        }
        if coordinator
            .owner_matches(thread_id, authentication.account_id)
            .await
            && thread.is_running()
        {
            return Ok(());
        ...
        let settings = thread.restorable_thread_settings().await;
        let client_mcp_extensions = thread.client_mcp_extensions();
        ...
        if thread.is_running() {
            if matches!(
                ...
                return Err(internal_error(format!(
                    "connection closed before thread {thread_id} could switch accounts"
                )));
            }
            ...
            match wait_for_thread_shutdown(&thread).await {
                ...
                ThreadShutdownResult::SubmitFailed => {
                    ...
                    return Err(internal_error(format!(
                        "failed to shut down thread {thread_id} before switching accounts"
                    )));
                }
                ThreadShutdownResult::TimedOut => {
                    thread_state.lock().await.take_shutdown_drain_waiter();
                    return Err(internal_error(format!(
                        "timed out shutting down thread {thread_id} before switching accounts"
                    )));
                }
            }
            tokio::time::timeout(Duration::from_secs(10), shutdown_drain_rx)
            .await
            .map_err(|_| {
                internal_error(format!(
                    "timed out waiting for thread {thread_id} shutdown events before switching accounts"
                ))
            })?
            .map_err(|_| {
                internal_error(format!(
                    "thread {thread_id} listener stopped before switching accounts"
                ))
            })?;
        }
        if !config.ephemeral
        ...
        {
            return Err(internal_error(format!(
                "thread {thread_id} changed while switching accounts"
            )));
        }
        self.outgoing
            .cancel_requests_for_thread(thread_id, /*error*/ None)
        ...
        let resumed = if config.ephemeral {
            self.thread_manager
                .resume_ephemeral_thread_with_auth_manager(
                    &thread,
        ...
        let NewThread {
            ...
            ..
        } = resumed.map_err(|error| {
            internal_error(format!(
                "failed to resume thread {thread_id} after switching accounts: {error}"
            ))
        })?;
        if resumed_thread_id != thread_id {
            return Err(internal_error(format!(
                "thread {thread_id} resumed as {resumed_thread_id} after switching accounts"
            )));
        }
        replacement_thread
            ...
            .map_err(|error| {
                internal_error(format!(
                    "failed to restore thread {thread_id} settings after switching accounts: {error}"
                ))
            })?;
        ...
        .await?;
        self.ensure_listener_task_running(thread_id, replacement_thread, thread_state)
            .await?;
        self.thread_watch_manager
            .upsert_thread(&thread_id.to_string())
            .await;
        coordinator
            .record_owner(thread_id, authentication.account_id)
            .await;
        Ok(())
    }
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            turnrail_coordinator: self.turnrail_coordinator.clone(),
    ...
    async fn thread_start_inner(
        ...
        let turnrail_coordinator = self.turnrail_coordinator.clone();
    ...
    async fn thread_start_task(
        ...
        let selected_authentication = match turnrail_coordinator.as_ref() {
            Some(coordinator) => Some(
                coordinator
                    .selected_authentication(&start_options.config, &start_options.config.cwd)
                    .await
                    .map_err(|error| {
                        internal_error(format!(
                            "failed to select the Codex Turnrail account: {error}"
        ...
        let selected_account_id = selected_authentication
            .as_ref()
            .map(|authentication| authentication.account_id);
        ...
        let new_thread = async {
            match selected_authentication {
                Some(authentication) => {
                    listener_task_context
                        .thread_manager
                        .start_thread_with_auth_manager(start_options, authentication.auth_manager)
                        .await
                }
                None => {
                    listener_task_context
        ...
        if let (Some(coordinator), Some(account_id)) = (turnrail_coordinator, selected_account_id) {
            coordinator.record_owner(thread_id, account_id).await;
        }
    ...
    async fn thread_revert_response(
        ...
        let runtime_snapshot = ThreadRevertRuntimeSnapshot {
            ...
            auth_manager: thread.auth_manager(),
    ...
    async fn thread_resume_inner(
        ...
        let resume_authentication = match self.turnrail_coordinator.as_ref() {
            Some(coordinator) => match coordinator
                .selected_authentication(&config, &config.cwd)
                .await
            {
        ...
        let resume_account_id = resume_authentication
            .as_ref()
            .map(|authentication| authentication.account_id);
        let resume_auth_manager = match resume_authentication {
            Some(authentication) => authentication.auth_manager,
            None => self.auth_manager.clone(),
        };
            ...
            .await
        {
            Ok(NewThread {
                ...
                if let (Some(coordinator), Some(account_id)) =
                    (self.turnrail_coordinator.as_ref(), resume_account_id)
                {
                    coordinator.record_owner(thread_id, account_id).await;
                }
    ...
    async fn thread_fork_inner(
        ...
        let config = self
            ...
            .await
            .map_err(|err| config_load_error(&err))?;
        let fork_authentication = match self.turnrail_coordinator.as_ref() {
            Some(coordinator) => Some(
                coordinator
                    .selected_authentication(&config, &config.cwd)
                    .await
        ...
        let fork_auth_manager = fork_authentication
            .as_ref()
            .map(|authentication| Arc::clone(&authentication.auth_manager));
        ...
        let new_thread = match (prepared_fork, fork_auth_manager) {
            (Some(prepared_fork), Some(auth_manager)) => {
                self.thread_manager
                    .fork_prepared_thread_with_auth_manager(
                        config,
                        prepared_fork,
                        thread_source,
                        parent_trace,
        ...
        if let (Some(coordinator), Some(authentication)) = (
            self.turnrail_coordinator.as_ref(),
            fork_authentication.as_ref(),
        ) {
            coordinator
                .record_owner(thread_id, authentication.account_id)
                .await;
