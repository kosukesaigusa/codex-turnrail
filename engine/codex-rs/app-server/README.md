...
## Turnrail directory routing

When `CODEX_TURNRAIL_ROOT` is set, the companion registry at `state.json` must use schema 3. Its `routing.defaultAccountIDs` and each `routing.directoryRules` entry define the only permitted accounts, in priority order. A directory entry contains `id`, an absolute `directory`, and `accountIDs`. Missing or invalid routing configuration fails explicitly.

At thread creation and before an idle `turn/start`, the Engine resolves the effective working directory and uses the most specific directory rule, or the configured default rule for unmatched directories. Verified linked Git worktrees inherit their main checkout's directory, including relative subdirectories. Selection changes apply to all conversations using that rule on their next turn. Active turns retain their authentication. Authentication and quota fallback never leave the selected rule; an empty rule blocks the request. No public JSON-RPC fields are added.
